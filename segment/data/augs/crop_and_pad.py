...
def random_crop_3D(img, label=None, crop_size=None):
    """
    Crop without respect to label
    :param img:
    :param label:
    :param crop_size: tuple(z_size, y_size, x_size)
    :return:
    """
    ...
    if crop_size[0] < img.shape[0]:
        ...
        img  = crop_and_pad_if_needed(img, axes=(0,), crop_size=crop_size[0])
        label = crop_and_pad_if_needed(label, axes=(0,), crop_size=crop_size[0])
        lb_z = 0
    ...
    if crop_size[1] < img.shape[1]:
        ...
        img = crop_and_pad_if_needed(img, axes=(1,), crop_size=crop_size[1])
        label = crop_and_pad_if_needed(label, axes=(1,), crop_size=crop_size[1])
        lb_y = 0
    ...
    if crop_size[2] < img.shape[2]:
        ...
        img = crop_and_pad_if_needed(img, axes=(2,), crop_size=crop_size[2])
        label = crop_and_pad_if_needed(label, axes=(2), crop_size=crop_size[2])
        lb_x = 0
